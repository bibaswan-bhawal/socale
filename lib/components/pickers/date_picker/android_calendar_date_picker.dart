...
const int _maxMonthPickerRowCount = 4; // 4 months per row.
// One extra row for the day-of-week header.
const double _maxDayPickerHeight = _dayPickerRowHeight * (_maxDayPickerRowCount + 1);
const double _maxMonthPickerHeight = _dayPickerRowHeight * (_maxMonthPickerRowCount + 1);
...
///    [AndroidCalendarDatePicker] and provides an optional compact view where the
///    user can enter a date as a line of text.
///  * [showTimePicker], which shows a dialog that contains a Material Design
///    time picker.
///
class AndroidCalendarDatePicker extends StatefulWidget {
  ...
  AndroidCalendarDatePicker({
    ...
    this.dateMode = AndroidDatePickerMode.dayMonthYear,
  })  : initialDate = DateUtils.dateOnly(initialDate),
        firstDate = DateUtils.dateOnly(firstDate),
        lastDate = DateUtils.dateOnly(lastDate),
        currentDate = DateUtils.dateOnly(currentDate ?? DateTime.now());

  final AndroidDatePickerMode dateMode;
  ...
  @override
  State<AndroidCalendarDatePicker> createState() => _AndroidCalendarDatePickerState();
}

class _AndroidCalendarDatePickerState extends State<AndroidCalendarDatePicker> {
  ...
  @override
  void didUpdateWidget(AndroidCalendarDatePicker oldWidget) {
      ...
      children: <Widget>[
        SizedBox(
          height: _subHeaderHeight +
              (widget.dateMode == AndroidDatePickerMode.dayMonthYear ? _maxDayPickerHeight : _maxMonthPickerHeight),
          child: _buildPicker(),
        ),
...
class _MonthPickerState extends State<_MonthPicker> {
  ...
  void didUpdateWidget(_MonthPicker oldWidget) {
    ...
    if (widget.initialMonth != oldWidget.initialMonth && widget.initialMonth != _currentMonth) {
      ...
      if (widget.dateMode == AndroidDatePickerMode.dayMonthYear) {
  ...
  Widget _buildMonthItem(BuildContext context, int index) {
    ...
    final bool isSelected = (month + 1) == widget.selectedDate.month;
    final bool isCurrentMonth =
        (month + 1) == widget.currentDate.month && widget.currentDate.year == widget.selectedDate.year;
    ...
    final Color? textColor = resolve<Color?>(
        (DatePickerThemeData? theme) => isCurrentMonth ? theme?.todayForegroundColor : theme?.yearForegroundColor,
        states);
    final Color? background = resolve<Color?>(
        (DatePickerThemeData? theme) => isCurrentMonth ? theme?.todayBackgroundColor : theme?.yearBackgroundColor,
        states);
      ...
      child: Container(
        ...
        child: Center(
          child: Semantics(
            selected: isSelected,
            button: true,
            child: Text(months[month], style: itemStyle),
          ),
        ),
      ),
    );
    ...
    monthItem = InkWell(
      key: ValueKey<int>(month + 1),
      onTap: () => widget.onChanged(DateTime(widget.selectedDate.year, month + 1)),
      ...
      child: Column(
        children: <Widget>[
          Container(
            padding: const EdgeInsetsDirectional.only(start: 16, end: 4),
            height: _subHeaderHeight,
            child: widget.dateMode == AndroidDatePickerMode.dayMonthYear
                ? Row(
                    children: <Widget>[
                      const Spacer(),
                      IconButton(
...
/// [AndroidCalendarDatePicker], or [showDatePicker] which create full date pickers.
///
/// See also:
///
///  * [AndroidCalendarDatePicker], which provides a Material Design date picker
///    interface.
///
  ...
  Widget _buildYearItem(BuildContext context, int index) {
    ...
    if (isDisabled) {
      ...
      yearItem = InkWell(
        ...
        onTap: () => widget.onChanged(DateTime(year, widget.selectedDate.month, widget.selectedDate.day)),
        statesController: MaterialStatesController(states),
        overlayColor: overlayColor,
        child: yearItem,
      );
