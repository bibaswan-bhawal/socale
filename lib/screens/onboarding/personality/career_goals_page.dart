...
class CareerGoalsPage extends ConsumerStatefulWidget {
  const CareerGoalsPage({Key? key}) : super(key: key);

  @override
  ConsumerState<CareerGoalsPage> createState() => _CareerGoalsPageState();
}

class _CareerGoalsPageState extends ConsumerState<CareerGoalsPage> {
  ...
  void _onClickEventHandler() {
    ...
    if (dataProvider.getCareerGoals.length < 3) {
      onboardingSnackBar.addMoreCareerGoalsSnack(context);
      return;
    }

    dataNotifier.uploadCareerGoals();
                    ...
                    Flexible(
                      child: Padding(
                        ...
                        child: CategoryChipSelectInput(
                          ...
                          onChange: dataNotifier.setCareerGoals,
                          map: careersOptionsList,
                          initValue: dataProvider.getCareerGoals,
                          gotData: dataProvider.getGotCareerGoalsData,
                        ),
                      ),
                    ),
                  ],
